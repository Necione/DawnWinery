"use client";

import { useSearchParams } from "next/navigation";
import type { DiscordUser } from "@/lib/discord";
import {
  getDiscordAvatarUrl,
  getDiscordDisplayName,
} from "@/lib/discord";

const DISCORD_ERROR_MESSAGES: Record<string, string> = {
  config: "Discord login is not set up yet.",
  state: "Login failed. Please try again.",
  exchange: "Could not verify your Discord account. Please try again.",
  access_denied: "Discord login was cancelled.",
  unauthorized: "Your Discord account does not have access to this dashboard.",
};

type DashboardAuthGateProps = {
  discordUser: DiscordUser | null;
  oauthConfigured: boolean;
};

function DiscordLogo() {
  return (
    <svg
      aria-hidden="true"
      viewBox="0 0 24 24"
      className="dashboard-auth-discord-icon"
    >
      <path d="M20.317 4.369A19.791 19.791 0 0 0 16.885 3.2a.074.074 0 0 0-.079.037 12.3 12.3 0 0 0-.608 1.243 18.224 18.224 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.243.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.331c-1.182 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.419 0 1.334-.956 2.419-2.157 2.419zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.419 0 1.334-.946 2.419-2.157 2.419z" />
    </svg>
  );
}

export function DashboardAuthGate({
  discordUser,
  oauthConfigured,
}: DashboardAuthGateProps) {
  const searchParams = useSearchParams();
  const discordError =
    searchParams.get("discord") === "error" && searchParams.get("reason")
      ? (DISCORD_ERROR_MESSAGES[searchParams.get("reason")!] ??
        "Discord login failed. Please try again.")
      : null;

  if (!oauthConfigured) {
    return (
      <div className="dashboard-auth-screen">
        <div className="dashboard-auth-card">
          <h1>Dashboard</h1>
          <p className="dashboard-auth-muted">
            Discord login is not configured. Add your Discord app credentials
            to enable access.
          </p>
        </div>
      </div>
    );
  }

  if (discordUser) {
    const displayName = getDiscordDisplayName(discordUser);

    return (
      <div className="dashboard-auth-screen">
        <div className="dashboard-auth-card">
          <h1>Access denied</h1>
          <p className="dashboard-auth-muted">
            Your Discord account is not authorized to view this dashboard.
          </p>
          <div className="dashboard-auth-user">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={getDiscordAvatarUrl(discordUser)}
              alt=""
              width={40}
              height={40}
              className="dashboard-auth-avatar"
            />
            <div className="dashboard-auth-user-meta">
              <strong>{displayName}</strong>
              <span>@{discordUser.username}</span>
            </div>
          </div>
          <a
            href="/api/auth/discord/clear?returnTo=/dashboard"
            className="dashboard-auth-link"
          >
            Use a different account
          </a>
        </div>
      </div>
    );
  }

  return (
    <div className="dashboard-auth-screen">
      <div className="dashboard-auth-card">
        <h1>Dashboard</h1>
        {discordError ? (
          <p className="dashboard-auth-error">{discordError}</p>
        ) : null}
        <a
          href="/api/auth/discord?returnTo=/dashboard"
          className="dashboard-auth-button"
        >
          <DiscordLogo />
          Sign in with Discord
        </a>
      </div>
    </div>
  );
}
